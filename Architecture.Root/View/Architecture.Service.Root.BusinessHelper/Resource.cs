    ...
    public partial class Resource
    {
        public GenericResponse<List<ResourceContract>> SelectByColumns(ObjectHelper objectHelper, ResourceRequest request)
        {
            GenericResponse<List<ResourceContract>> returnObject = objectHelper.InitializeGenericResponse<List<ResourceContract>>("SelectByColumns");
             Architecture.DataAccess.Root.BusinessHelper.Resource dac = new Architecture.DataAccess.Root.BusinessHelper.Resource();
            
            GenericResponse<List<ResourceContract>> response = dac.SelectByColumns(request.UserName,request.Password,request.ResourceId,request.ResourceCode);
            if (!response.Success)
            {
            ...
            {
                GenericResponse<List<ResourceActionContract>> responseAction = dac.SelectActionByResourceId(item.ResourceId);
